var YAO = function(){
	...
	return {
		...
		Builder: {
			...
			// è¿½åŠ LinkèŠ‚ç‚¹ï¼ˆæ·»åŠ CSSæ ·å¼è¡¨ï¼‰
		...
		ajax: function(config){
			var oXhr, method = config.method ? config.method.toUpperCase() : 'GET', url = config.url || '', fn = config.fn || null, postData = config.data || null, elem = config.id ? YAO.getEl(config.id) : (config.element || null), load = config.loadFn ? config.loadFn : (config.loading || 'æ­£åœ¨èŽ·å–æ•°æ®ï¼Œè¯·ç¨åŽ...');
			if (!url) {
				return;
			}
...
YAO.YLightBox = {
	...
	minimize: function(){
		...
		if (this.YBoxMaximizeBar) {
			this.YBoxMaximizeBar.title = 'Original';
		}
